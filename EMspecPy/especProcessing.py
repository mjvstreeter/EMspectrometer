...
from EMspecPy.general_tools import load_object
...
class Espec_proc():
    ...
    def file2screen(self,file_path):
        ...
        img_pC_permm2 = self.data2screen(img_raw)
        return img_pC_permm2

    def data2screen(self,img_raw):
    ...
    def background_sub(self,img_raw):
    ...
    def screen2spec(self,img_screen):
    ...
    def energy_spectrum(self,img_raw):
        img_pC_permm2 = self.data2screen(img_raw)
        spec_pC_per_mm_per_MeV = self.screen2spec(img_pC_permm2)
...
class Espec_ang_proc():
    ...
    def screen2spec(self,img_screen,theta):
        """ convert image to spectrum
        Uses 1d interpolation along horrizontal axis (1)
        """
        if np.size(theta)==1:
